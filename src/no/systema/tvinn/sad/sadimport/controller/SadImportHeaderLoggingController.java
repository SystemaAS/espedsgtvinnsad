...
public class SadImportHeaderLoggingController {
	...
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			//Debug --> 
		    	...
		    	if(jsonPayload!=null){
		    		JsonSadImportSpecificTopicLoggingContainer container = this.sadImportSpecificTopicService.getSadImportSpecificTopicLoggingContainer(jsonPayload);
	...
	@RequestMapping(value="tvinnsadimport_renderXml.do", method={ RequestMethod.GET })
	public ModelAndView doSadImportRenderXml(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doSadImportRenderXml...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
