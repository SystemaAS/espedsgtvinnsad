...
@Scope("session")
public class SadNcts5ExportController {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = LoggerFactory.getLogger(SadNcts5ExportController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadncts5export", params="action=doFind",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doFind(@ModelAttribute ("record") SearchFilterSadNctsExportTopicList recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("tvinnsadncts5export");
		...
		if(appUser==null){
			...
			if(bindingResult.hasErrors()){
	            ...
	            //get BASE URL
	    		String BASE_URL = SadNctsExportUrlDataStore.NCTS5_EXPORT_BASE_TOPICLIST_URL;
	    		//only when docRef exists
	    		if(searchFilter!=null && strMgr.isNotNull(searchFilter.getDocRef())){
	    			BASE_URL = SadNctsExportUrlDataStore.NCTS5_EXPORT_BASE_TOPICLIST_DOCREF_URL;
	    		}
	    		//add URL-parameters
				...
				if(jsonPayload!=null){
					...
					logger.debug("SESSION ID from Controller:" + session.getId());
