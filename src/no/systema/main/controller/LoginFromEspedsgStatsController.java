...
public class LoginFromEspedsgStatsController {
	...
	public ModelAndView logon(@ModelAttribute (AppConstants.SYSTEMA_WEB_USER_KEY) SystemaWebUser appUser, BindingResult bindingResult, HttpSession session, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttr){
		...
		logger.info("SIGNATUR:" + appUser.getPassword());
		logger.info("EXPORT:" + sadExport);
		//default
		String goToView = "tvinnsadimport_edit.do;";
		//when export
		if(strMgr.isNotNull(sadExport)){ 
			goToView = "tvinnsadexport_edit.do;";
			appUser.setActiveMenu(SystemaWebUser.ACTIVE_MENU_TVINN_SAD_EXPORT);
		}
		ModelAndView successView = new ModelAndView("redirect:" + goToView + "?action=doFetch" + "&avd=" + avd + "&opd=" + opd + "&sg=" + signatur);
	...
	*/
	...
	private String getTomcatServerRedirectionUrl(SystemaWebUser appUser, HttpServletRequest request){
		...
		try{
			retval = hostRaw + request.getContextPath() + "/logonWRedAnalyseReports_toSad.do?" + "ru=" + appUser.getUser() + "&dp=" + URLEncoder.encode(appUser.getEncryptedPassword(), "UTF-8");
		}catch(Exception e){
